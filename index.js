...
function openPanel() {
    ...
    // 關閉 SillyTavern 的擴展選單抽屜（這樣才不會擋住）
...
function renderLoginScreen(container) {
    container.innerHTML = `
        <div class="baozi-login">
            ...
            <div class="baozi-form">
                <label>你的暱稱</label>
                <input type="text" id="baozi-nickname-input" placeholder="例如：包子" value="${localStorage.getItem('baozi_nickname') || ''}">
                <label>圈內暗號</label>
                <input type="password" id="baozi-secret-input" placeholder="只有圈內人知道的那個～">
                <button class="baozi-btn-primary" id="baozi-login-btn">進入同好會 ✨</button>
                <div class="baozi-error" id="baozi-login-error"></div>
            </div>
        </div>
    `;
...
function renderHomeScreen(container) {
    container.innerHTML = `
        <div class="baozi-home">
            <div class="baozi-greeting">你好，<b>${state.nickname}</b>～今天也辛苦啦 💕</div>
            <div class="baozi-menu">
                <button class="baozi-menu-btn" data-target="my-commands">
                    <span class="baozi-menu-icon">📚</span>
                    <span>我的指令庫</span>
                </button>
                <button class="baozi-menu-btn" data-target="shared-space">
                    <span class="baozi-menu-icon">🏠</span>
                    <span>共享空間</span>
                </button>
                <button class="baozi-menu-btn" data-target="shared-chats">
                    <span class="baozi-menu-icon">💬</span>
                    <span>分享的對話</span>
                </button>
                <button class="baozi-menu-btn" data-target="settings">
                    <span class="baozi-menu-icon">⚙️</span>
                    <span>設定</span>
                </button>
            </div>
        </div>
    ...
    container.innerHTML = `
        <div class="baozi-screen">
            <div class="baozi-screen-header">
                <button class="baozi-back" id="baozi-back">← 回首頁</button>
                <h3>📚 我的指令庫</h3>
            </div>
            <button class="baozi-btn-primary baozi-btn-full" id="baozi-add-cmd">+ 新增指令</button>
            <div class="baozi-list" id="baozi-cmd-list">
                <div class="baozi-loading">載入中...</div>
            </div>
        </div>
    `;
            ...
            <div class="baozi-card">
                ...
                <div class="baozi-card-actions">
                    ${cmd.shared
                        ...
                        : `<button class="baozi-btn-small" data-action="share" data-id="${cmd.id}">📤 分享到共享空間</button>`}
        ...
        list.querySelectorAll('[data-action]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (action === 'share') {
                    ...
                    navigator.clipboard.writeText(cmd.content);
                    btn.textContent = '✓ 已複製';
...
function renderAddCommandScreen(container) {
    container.innerHTML = `
        <div class="baozi-screen">
            ...
            <div class="baozi-form">
                <label>標題</label>
                <input type="text" id="baozi-new-title" placeholder="例如：雨夜咖啡廳場景">
                <label>內容</label>
                <textarea id="baozi-new-content" rows="8" placeholder="貼上你的指令、劇本、prompt..."></textarea>
                <label>標籤（選填，用逗號分隔）</label>
                <input type="text" id="baozi-new-tags" placeholder="例如：雨天, 咖啡廳, 甜">
                <button class="baozi-btn-primary baozi-btn-full" id="baozi-save-cmd">💾 儲存</button>
                <div class="baozi-error" id="baozi-save-error"></div>
            </div>
        </div>
    `;
...
async function renderSharedSpaceScreen(container) {
    container.innerHTML = `
        <div class="baozi-screen">
            ...
            <div class="baozi-list" id="baozi-shared-list">
                <div class="baozi-loading">載入中...</div>
            </div>
        </div>
    `;
...
async function renderSharedChatsScreen(container) {
    container.innerHTML = `
        <div class="baozi-screen">
            ...
            <button class="baozi-btn-primary baozi-btn-full" id="baozi-new-chat">+ 分享一段對話</button>
            <div class="baozi-list" id="baozi-chat-list">
                <div class="baozi-loading">載入中...</div>
            </div>
        </div>
    `;
    ...
    try {
        ...
        list.innerHTML = chats.map(chat => `
            <div class="baozi-card">
                <div class="baozi-card-owner">${escapeHtml(chat.ownerNickname || '匿名')}</div>
        ...
        list.querySelectorAll('[data-action="comment"]').forEach(btn => {
            btn.addEventListener('click', () => {
                toggleCommentPanel('chat', btn.dataset.id);
            });
        });
    } catch (err) {
        list.innerHTML = '<div class="baozi-error">載入失敗：' + err.message + '</div>';
...
function renderShareChatScreen(container) {
    // 嘗試從 SillyTavern 取當前對話
    const stChat = extractCurrentChat();
    ...
    container.innerHTML = `
        <div class="baozi-screen">
            ...
            <div class="baozi-form">
                <label>標題</label>
                <input type="text" id="baozi-chat-title" placeholder="例如：今天跟角色的離譜對話">
                <label>前言（吐槽、感想）</label>
                <textarea id="baozi-chat-preface" rows="2" placeholder="例如：你看這段！！真的笑死🤣"></textarea>
                <label>對話內容（可以貼上來、或編輯）</label>
                <textarea id="baozi-chat-content" rows="10" placeholder="貼上要分享的對話片段...">${escapeHtml(stChat)}</textarea>
                ...
                <div class="baozi-preview" id="baozi-preview-label">預覽：</div>
                <div class="baozi-preview-box" id="baozi-preview-box"></div>
                <button class="baozi-btn-primary baozi-btn-full" id="baozi-send-chat">📤 發送分享</button>
    ...
    document.getElementById('baozi-send-chat').addEventListener('click', async () => {
        ...
        try {
            ...
            errBox.textContent = '發送失敗：' + err.message;
        }
    });
}
...
function renderSettingsScreen(container) {
    container.innerHTML = `
        <div class="baozi-screen">
            ...
            <div class="baozi-form">
                <label>你的暱稱</label>
                <input type="text" id="baozi-edit-nickname" value="${escapeHtml(state.nickname || '')}">
                <button class="baozi-btn-primary" id="baozi-save-nickname">儲存暱稱</button>
                <hr style="margin: 20px 0; border-color: rgba(0,0,0,0.1);">
                <div class="baozi-info">
                    <div><b>暱稱：</b>${escapeHtml(state.nickname)}</div>
                    <div><b>使用者 ID：</b><span style="font-size:11px; opacity:.6;">${state.user.uid}</span></div>
                </div>
                <hr style="margin: 20px 0; border-color: rgba(0,0,0,0.1);">
                <button class="baozi-btn-small baozi-btn-full" id="baozi-reset-all" style="margin-bottom:10px;">🔧 重新設定 Firebase / 暗號</button>
                <button class="baozi-btn-danger baozi-btn-full" id="baozi-logout">登出</button>
            </div>
        </div>
    `;
